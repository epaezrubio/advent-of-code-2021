...
class CaveGraph {
  ...
  public getAllRoutes(
    ...
    if (source.name === "start" && visitedCaves.length > 0) {
      return [];
    }
    ...
    const isSmallCaveAlreadyVisited =
      source.small && visitedCaves.includes(source);

    // small caves cannot be visited twice more than once
    if (isSmallCaveAlreadyVisited && hasVisitedSmallCaveTwice) {
      return [];
    }
    ...
    for (const connection of source.connections) {
      ...
      routes.push(
        ...this.getAllRoutes(
          connection,
          newVisitedCaves,
          hasVisitedSmallCaveTwice || isSmallCaveAlreadyVisited
        )
      );
    }
