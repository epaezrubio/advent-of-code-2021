...
function getExpandedInput(grid: number[][], expansion: number): number[][] {
  const expandedGrid: number[][] = [];

  for (let i = 0; i < grid.length * expansion; i++) {
    const expandedRow: number[] = [];
    const iE = (i / grid.length) >> 0;

    const copyRowIndex = i % grid.length;

    for (let j = 0; j < grid[copyRowIndex].length * expansion; j++) {
      const copyCellIndex = j % grid[copyRowIndex].length;
      const jE = (j / grid[copyRowIndex].length) >> 0;

      const newVal = grid[copyRowIndex][copyCellIndex] + iE + jE;
      const clampedNewVal = newVal > 9 ? newVal - 9 : newVal;

      expandedRow[j] = clampedNewVal;
    }

    expandedGrid[i] = expandedRow;
  }
...
const expandedInput = getExpandedInput(input, EXPANSION_MULTIPLIER);

console.log(getLowestRiskPath(expandedInput));
